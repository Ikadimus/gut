...
import { Users, Plus, Trash2, Mail, UserPlus, Shield, Loader2, X, Save, Edit, Terminal } from 'lucide-react';
...
export const AdminUsers: React.FC<AdminUsersProps> = ({ currentUser }) => {
  ...
  const handleEdit = (user: User) => {
    // Lógica de permissão para editar
    if (!canEdit(user)) {
      alert("Você não tem permissão para editar este nível de acesso.");
      return;
    }
  ...
  const handleDelete = async (user: User) => {
    if (!canDelete(user)) {
      alert("Este usuário é protegido ou possui nível superior ao seu.");
      return;
    }
  ...
  const resetForm = () => {
    setName(''); setEmail(''); setPassword(''); setRole(UserRole.VIEWER);
  };

  // Regras de Negócio de Permissão
  const canEdit = (target: User) => {
    // Desenvolvedor edita todo mundo
    if (currentUser.role === UserRole.DEVELOPER) return true;
    
    // Todo mundo edita a si mesmo
    if (currentUser.id === target.id) return true;

    // Administrador edita apenas Editor e Viewer (não edita outros Admins nem o Dev)
    if (currentUser.role === UserRole.ADMIN) {
      return target.role === UserRole.EDITOR || target.role === UserRole.VIEWER;
    }
  ...
  const canDelete = (target: User) => {
    // Ninguém deleta o desenvolvedor master
    if (target.email === 'efilho@essencisbiometano.com.br') return false;
    
    // Desenvolvedor deleta qualquer um menos a si mesmo (proteção básica)
    if (currentUser.role === UserRole.DEVELOPER) return currentUser.id !== target.id;

    // Administrador deleta apenas Editor e Viewer
    if (currentUser.role === UserRole.ADMIN) {
      return target.role === UserRole.EDITOR || target.role === UserRole.VIEWER;
    }
  ...
  // Filtra as opções de papel que o usuário pode atribuir
      ...
      {showAddForm && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 animate-slide-up ring-1 ring-white/5">
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Nível</label>
              <select value={role} onChange={e => setRole(e.target.value as UserRole)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 outline-none">
              ...
              <tr key={u.id} className="hover:bg-slate-800/30 transition-all">
                ...
                <td className="px-6 py-4">
                  <span className={`mx-auto px-3 py-1 rounded-full text-[9px] font-black uppercase border flex items-center gap-1 w-fit
                    ${u.role === UserRole.DEVELOPER ? 'text-purple-400 border-purple-800/50' :
                      u.role === UserRole.ADMIN ? 'text-red-400 border-red-800/50' : 
                      u.role === UserRole.EDITOR ? 'text-blue-400 border-blue-800/50' : 'text-slate-400 border-slate-700'}`}>
                    {u.role === UserRole.DEVELOPER ? <Terminal size={10} /> : null}
                    {u.role}
                  </span>
                </td>
                <td className="px-6 py-4 flex justify-center gap-2">
                  <button 
                    ...
                    disabled={!canEdit(u)}
                    className={`p-2 rounded-lg transition-all ${canEdit(u) ? 'text-blue-400 hover:bg-blue-900/20' : 'text-slate-700 cursor-not-allowed'}`}
                  >
                    <Edit size={16} />
                  </button>
                  <button 
                    ...
                    className={`p-2 rounded-lg transition-all ${canDelete(u) ? 'text-red-400 hover:bg-red-900/20' : 'text-slate-700 cursor-not-allowed'}`}
                  >
                    <Trash2 size={16} />
                  </button>
                </td>
