...
export const AreaManager: React.FC<AreaManagerProps> = ({ areas, onUpdateAreas, onCancel }) => {
  ...
  return (
    <div className="bg-slate-800 rounded-lg shadow-lg p-6 max-w-4xl mx-auto border border-slate-700">
        ...
        <div className="flex gap-3 mb-6 p-4 bg-slate-900/50 rounded-lg border border-slate-700">
            <input 
                type="text" 
                value={newArea}
                    ...
                    <li key={index} className="p-4 flex items-center justify-between hover:bg-slate-700/50 transition-colors">
                        {editingIndex === index ? (
                            <div className="flex-1 flex gap-2 mr-4">
                                ...
                                />
                                <button onClick={() => saveEdit(index)} className="text-green-400 hover:bg-green-900/30 p-2 rounded"><Save size={18}/></button>
                                <button onClick={() => setEditingIndex(null)} className="text-red-400 hover:bg-red-900/30 p-2 rounded"><X size={18}/></button>
                            </div>
                        ) : (
                        ...
                        {editingIndex !== index && (
                            <div className="flex gap-1">
                                <button onClick={() => startEdit(index, area)} className="text-slate-500 hover:text-blue-400 hover:bg-blue-900/30 p-2 rounded transition-colors" title="Editar">
                                    <Edit2 size={18} />
                                </button>
                                <button onClick={() => handleDelete(index)} className="text-slate-500 hover:text-red-400 hover:bg-red-900/30 p-2 rounded transition-colors" title="Excluir">
                                    <Trash2 size={18} />
                                </button>
                            </div>
                        )}
