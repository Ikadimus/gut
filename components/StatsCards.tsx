...
export const StatsCards: React.FC<StatsCardsProps> = ({ 
  ...
  // Total de Riscos Unificados Pendentes
  const totalCriticalRisks = criticalGUTCount + criticalThermoCount;

  // Lógica de Concentração de Risco GUT (Apenas para Pendências)
  const areaImpact = activeIssues.reduce((acc, issue) => {
    acc[issue.area] = (acc[issue.area] || 0) + issue.score;
    return acc;
  }, {} as Record<string, number>);

  const areaRecurrence = activeIssues.reduce((acc, issue) => {
    acc[issue.area] = (acc[issue.area] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const criticalArea = Object.entries(areaImpact).sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0] || 'Nominal';
  const instableArea = Object.entries(areaRecurrence).sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0] || 'Estável';
    ...
    <div className="space-y-6 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 flex items-center shadow-lg group hover:border-blue-500/30 transition-all">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Pendências Ativas</p>
        ...
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 flex items-center shadow-lg group hover:border-red-500/30 transition-all">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Críticos Atuais</p>
        ...
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 flex items-center shadow-lg group hover:border-green-500/30 transition-all">
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Ações Concluídas</p>
            <h3 className="text-2xl font-black text-slate-100">{resolvedCount}</h3>
          </div>
        </div>
      </div>

      {/* Linha 2 e 3 de Concentração de Risco */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className={`bg-slate-950/40 border p-6 rounded-3xl flex items-center gap-6 shadow-xl relative overflow-hidden group transition-all ${totalOpen > 0 ? 'border-orange-950/30' : 'border-slate-800'}`}>
          <div className={`p-4 rounded-2xl border shadow-lg relative z-10 ${totalOpen > 0 ? 'bg-orange-500/10 border-orange-500/20 text-orange-500' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
            <Target size={28} />
          </div>
          <div className="relative z-10">
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mb-1">Concentração de Risco Pendente</p>
            <h4 className="text-xl font-black text-white uppercase tracking-tight">{criticalArea}</h4>
            <p className="text-[9px] text-slate-600 font-bold uppercase mt-1">Setor com maior impacto acumulado em aberto</p>
          </div>
        </div>

        <div className={`bg-slate-950/40 border p-6 rounded-3xl flex items-center gap-6 shadow-xl relative overflow-hidden group transition-all ${totalOpen > 0 ? 'border-purple-950/30' : 'border-slate-800'}`}>
          <div className={`p-4 rounded-2xl border shadow-lg relative z-10 ${totalOpen > 0 ? 'bg-purple-500/10 border-purple-500/20 text-purple-500' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
            <Zap size={28} />
          </div>
          <div className="relative z-10">
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mb-1">Área com Falhas Recorrentes (Ativas)</p>
            <h4 className="text-xl font-black text-white uppercase tracking-tight">{instableArea}</h4>
            <p className="text-[9px] text-slate-600 font-bold uppercase mt-1">Volume de registros pendentes por subsistema</p>
          </div>
        </div>
      </div>

      {/* Linha Final: Saúde do Processo */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 relative overflow-visible shadow-xl">
           <div className="flex items-center gap-6">
              <div className={`p-4 rounded-2xl bg-slate-950 border border-slate-800 shadow-inner ${hasRisks ? 'text-red-500' : 'text-green-500'}`}>
                  <Gauge size={32} />
              </div>
              <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between">
                    <h4 className={`text-lg font-black uppercase tracking-tight ${hasRisks ? 'text-red-500' : 'text-green-500'}`}>
                      {!hasRisks ? 'Operação Nominal (Sem Riscos Pendentes)' : `${totalCriticalRisks} Riscos Críticos em Aberto`}
                    </h4>
                    {hasRisks && (
                      <button 
           ...
           {showActiveRisks && hasRisks && (
             <div className="absolute top-full left-0 right-0 mt-2 z-[60] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-4 animate-slide-up max-h-80 overflow-y-auto custom-scrollbar ring-1 ring-white/10">
                <h5 className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 border-b border-slate-800 pb-2">Inventário de Riscos Críticos Ativos</h5>
                <div className="space-y-2">
                   {/* Riscos GUT Pendentes */}
                   {activeIssues.filter(i => i.score >= 81).map(issue => (
                     <div key={issue.id} className="group/item flex items-center justify-between bg-red-950/20 p-2.5 rounded-xl border border-red-900/30 hover:bg-red-900/40 transition-all">
                        <div className="flex items-center gap-3 overflow-hidden">
                           ...
                           <div className="flex flex-col overflow-hidden">
                              ...
                              <span className="text-[8px] text-slate-500 font-bold uppercase">{issue.area} (GUT)</span>
                           </div>
                        </div>
                     ...
                     <div key={`thermo-${record.id}`} className="group/item flex items-center justify-between bg-orange-950/20 p-2.5 rounded-xl border border-orange-900/30 hover:bg-orange-900/40 transition-all">
                        <div className="flex items-center gap-3 overflow-hidden">
                           ...
                           <div className="flex flex-col overflow-hidden">
                              ...
                              <span className="text-[8px] text-slate-500 font-bold uppercase">{record.area} (Térmico)</span>
                           </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="text-[9px] font-black text-orange-400 bg-orange-900/40 px-2 py-0.5 rounded border border-orange-800/30">{record.currentTemp}°C</span>
        ...
        <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 flex items-center gap-6 shadow-xl">
           ...
           <div>
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mb-1">Disponibilidade Estimada</p>
              <h4 className="text-lg font-black text-slate-100 uppercase tracking-tight">
                {Math.max(0, 99.8 - (totalOpen * 0.5) - (totalCriticalRisks * 2.5)).toFixed(1)}% Nom.
              </h4>
              <p className="text-[9px] text-slate-600 font-bold uppercase mt-1">Algoritmo baseado em pendências em aberto</p>
           </div>
        </div>
      </div>
    </div>
