...
export const IssueForm: React.FC<IssueFormProps> = ({ onSave, onCancel, areas }) => {
  ...
  if (currentScore > 60) scoreColor = 'bg-amber-900/40 text-amber-200 border-amber-800';
  if (currentScore > 90) scoreColor = 'bg-red-900/40 text-red-200 border-red-800';

  return (
    <div className="bg-slate-800 rounded-lg shadow-lg p-6 max-w-4xl mx-auto border border-slate-700">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-slate-100">Novo Registro de Ocorrência</h2>
        <button onClick={onCancel} className="text-slate-400 hover:text-slate-200">Cancelar</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Área da Planta</label>
              <select 
                value={area} 
                onChange={(e) => setArea(e.target.value)}
                className="w-full rounded-md bg-slate-700 border-slate-600 text-slate-100 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border"
              >
                {areas.map(a => (
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-md bg-slate-700 border-slate-600 text-slate-100 placeholder-slate-400 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border"
             ...
             <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                <div className="flex items-center justify-between mb-2">
                    <h4 className="font-semibold text-slate-300 flex items-center gap-2">
                        <Bot size={18} className="text-purple-400"/> Assistente IA
                    </h4>
                    <button
                        type="button"
                        onClick={handleAISuggestion}
                        disabled={aiLoading}
                        className="text-xs bg-purple-700 hover:bg-purple-600 text-white px-3 py-1.5 rounded-full flex items-center gap-1 transition-colors disabled:opacity-50"
                    >
                ...
                {aiReasoning && (
                    <div className="bg-purple-900/30 text-purple-200 text-sm p-3 rounded mt-2 border border-purple-800/50">
          ...
          <div className="space-y-6 bg-slate-900/50 p-6 rounded-lg border border-slate-700">
             <h3 className="font-bold text-slate-200 border-b border-slate-700 pb-2 mb-4">Matriz GUT</h3>
             
             <div>
                <label className="flex justify-between text-sm font-medium text-slate-300 mb-1">
                    <span>Gravidade (G)</span>
                    <span className="font-bold text-slate-100">{gravity}</span>
                </label>
                <input 
             ...
             <div>
                <label className="flex justify-between text-sm font-medium text-slate-300 mb-1">
                    ...
                    <span className="font-bold text-slate-100">{urgency}</span>
                </label>
                <input 
                    type="range" min="1" max="5" step="1"
             ...
             <div>
                <label className="flex justify-between text-sm font-medium text-slate-300 mb-1">
                    ...
                    <span className="font-bold text-slate-100">{tendency}</span>
                </label>
                <input 
                    type="range" min="1" max="5" step="1"
             ...
             <div className="pt-4 mt-4 border-t border-slate-700">
                <div className="flex justify-between items-center">
                    <span className="text-lg font-bold text-slate-300">Pontuação GUT Total:</span>
                    <span className={`text-3xl font-black px-4 py-2 rounded-lg border ${scoreColor}`}>
                        {currentScore}
                    </span>
                </div>
                <p className="text-xs text-right text-slate-500 mt-1">G x U x T</p>
             </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-700">
            <button
                type="button"
                onClick={onCancel}
                className="px-4 py-2 text-slate-300 bg-slate-700 border border-slate-600 rounded-lg hover:bg-slate-600 font-medium transition-colors"
            >
                Cancelar
            </button>
            <button
                type="submit"
                className="px-4 py-2 text-white bg-green-600 rounded-lg hover:bg-green-700 font-medium flex items-center gap-2 transition-colors"
            >
